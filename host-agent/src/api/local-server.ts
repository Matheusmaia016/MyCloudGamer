...
const remoteStartSchema = z.object({
  backendSessionId: z.string().min(1),
  gameId: z.string().min(1),
  executablePath: z.string().optional(),
});
...
export class LocalAgentServer {
  ...
  constructor(
    ...
    private readonly getHostGames: () => Promise<HostGame[]>,
  ...
  private registerRoutes() {
    ...
    this.app.get('/games/library', async () => ({ games: await this.getHostGames() }));
    ...
    this.app.post('/session/start', async (request, reply) => {
      try {
        ...
        const session = await this.sessionService.startSession(body);
    ...
    this.app.post('/session/remote-start', async (request, reply) => {
      try {
        const body = remoteStartSchema.parse(request.body);
        await this.sessionService.startSession({ gameId: body.gameId, executablePath: body.executablePath });
        await this.sessionService.startRemoteSessionMock(body.backendSessionId, (status) =>
          backendApi.updateSessionStatus(body.backendSessionId, status).then(() => undefined),
        );

        return reply.send({ ok: true });
      } catch (error) {
        logger.error({ error }, 'Erro ao processar comando remote-start');
        return reply.status(400).send({ message: error instanceof Error ? error.message : 'Erro ao executar remote-start' });
      }
    });
