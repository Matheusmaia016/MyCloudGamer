...
export class HostAuthService {
  ...
  public async loginOrRegisterHost(): Promise<HostInfo> {
    ...
    logger.info('Iniciando pareamento do host com backend');

    const result = await backendApi.confirmPairing({
      code: env.PAIRING_CODE,
    ...
    logger.info({ hostId: result.host.id, hostName: result.host.name }, 'Host autenticado/pareado com sucesso');
